...
export function formatViews(num: number, precision = 1) {
  ...
  if (found) {
    const formatted = (num / found.threshold).toFixed(precisionToUse) + found.suffix;
    return formatted;
  }
...
export function featureNotReady(featureName: string, message?: string) {
  const toastMessage = message ? message : "This feature is not yet implemented.";
...
export const generateRandId = (prefix = "", length = 8) => {
